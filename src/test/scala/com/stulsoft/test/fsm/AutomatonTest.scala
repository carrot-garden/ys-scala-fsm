...
class AutomatonTest extends FlatSpec with Matchers {
	...
	"An Automaton" should "support transitions - one" in {
		...
		val nextState = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(1.0)))), transitions)
		nextState should be(State("finish"))

		val nextStateNull = Automaton.nextState(State("notExisting"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(1.0)))), transitions)
		nextStateNull should be(null)

		val nextStateNull2 = Automaton.nextState(State("start"), Input(InputType("NONtest"), InputParams(Map("P1" -> ParamDouble(1.0)))), transitions)
		nextStateNull2 should be(null)
	}
	it should "support priorities" in {
		val transitions = buildTransitionsTwo().sortBy { x => x.priority }
		val nextState = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P2" -> ParamInt(2)))), transitions)
		nextState should be(State("finish10"))
	}
	it should "support aggregation type all" in {
		val transitions = buildTransitionsOneTransitionManyConditions().sortBy { x => x.priority }
		val nextStateFault = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2)))), transitions)
		nextStateFault should be(null)

		val nextStateSuccess = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2), "P2" -> ParamDouble(3.0)))), transitions)
		nextStateSuccess should be(State("finish"))

		val nextStateFault2 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2), "P2" -> ParamDouble(3.0001)))), transitions)
	...
	it should "support aggregation type one" in {
		...
		val nextStateSuccess1 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2)))), transitions)
		nextStateSuccess1 should be(State("finish"))

		val nextStateSuccess2 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2), "P2" -> ParamDouble(3.0)))), transitions)
		nextStateSuccess2 should be(State("finish"))

		val nextStateSuccess3 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2), "P2" -> ParamDouble(3.0001)))), transitions)
		nextStateSuccess3 should be(State("finish"))
	}
	it should "support aggregation type notOne" in {
		...
		val nextStateFault1 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2)))), transitions)
				nextStateFault1 should be(null)
				
				val nextStateFault2 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2), "P2" -> ParamDouble(3.0)))), transitions)
				nextStateFault2 should be(null)
				
				val nextStateFault3 = Automaton.nextState(State("start"), Input(InputType("test"), InputParams(Map("P1" -> ParamDouble(2), "P2" -> ParamDouble(3.0001)))), transitions)
